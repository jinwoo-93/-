...
import { postUpdateSchema, type PostUpdateInput } from '@/lib/validations';
...
export default function EditPostPage() {
  ...
  const fetchPost = async () => {
    try {
      ...
      if (!data.success) {
        toast({
          title: language === 'ko' ? '게시글을 찾을 수 없습니다' : '找不到商品',
          variant: 'destructive',
        });
        router.push('/posts');
        return;
      ...
      if (post.userId !== user?.id) {
        toast({
          title: language === 'ko' ? '수정 권한이 없습니다' : '无修改权限',
          variant: 'destructive',
        });
      ...
      setValue('priceKRW', post.priceKRW);
      ...
      // 배송 설정
      if (post.shippingCompanyId) {
        setSelectedCompanyId(post.shippingCompanyId);
      }
      if (post.shippingFeeType) {
        setShippingFeeType(post.shippingFeeType);
      }
      if (post.shippingFeeAmount) {
        setShippingFeeAmount(post.shippingFeeAmount.toString());
      }
      if (post.freeShippingThreshold) {
        setFreeShippingThreshold(post.freeShippingThreshold.toString());
      }
  ...
  const onSubmit = async (data: PostUpdateInput) => {
    ...
    // 판매 게시글인 경우 배송 설정 검증
    if (data.postType === 'SELL' || originalPost?.postType === 'SELL') {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: language === 'ko' ? '게시글이 수정되었습니다' : '商品已修改' });
        router.push(`/posts/${postId}`);
      } else {
  ...
  return (
    <div className="container-app py-6 max-w-2xl">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {/* 배송 설정 (판매 게시글만) */}
        {(postType === 'SELL' || originalPost?.postType === 'SELL') && (
          <Card>
            <CardHeader>
